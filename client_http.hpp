        ...
        if(!ec) {
          ...
          if(header_it != session->response->header.end()) {
            ...
            if(content_length > num_additional_bytes) {
              session->connection->set_timeout();
              asio::async_read(*session->connection->socket, session->response->streambuf, asio::transfer_exactly(static_cast<uint32_t>(content_length - static_cast<std::size_t>(num_additional_bytes))), [session](const error_code &ec, std::size_t /*bytes_transferred*/) {
                session->connection->cancel_timeout();
                auto lock = session->connection->handler_runner->continue_lock();
                if(!lock)
                  return;
